...
Like `args` and `services` the `auth` property on the `Props` is a placholder object you can use to store authentication data about the current request. Auth hooks (like `useJWTAuth`, `useBasicAuth`, and `useApiKey`) do not populate the args, they populate the `auth` property.
